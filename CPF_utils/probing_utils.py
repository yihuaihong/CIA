...
# 全局路径
SAVE_DIR = "${SCRATCH}/results/open-r1/probing_results"
...
def run_two_hop_linear_probe_evaluation(model, train_dataset, eval_dataset, train_dataset_name,
    ...
    if per_sample_info is not None and cot_pred_bridges is not None:
        ...
        labeled_path = (
            f"${SCRATCH}/results/open-r1/twohop_results/"
    ...
    # ── 保存结果到 jsonl ──────────────────────────────────────────────────
    output_dir = f'${SCRATCH}/results/open-r1/{eval_dataset_name}_{model_name}_{interp_tool}_{seed}_cpf_results'
