        ...
        if interp_tool == 'logit_lens':
            ...
            if dataset_name in ['TwoHopFact', 'HoppingtooLate', 'SOCRATES']:
                ...
                CPF_result = run_logit_lens_evaluation(
                    ...
                    eval_dataset_responses_path=f'${SCRATCH}/results/open-r1/twohop_results/{dataset_name}_{model_name}_{seed}_results.jsonl',
                    tokenizer=tokenizer,
                    batch_size=batch_size,
                    output_path=f'${SCRATCH}/results/open-r1/{dataset_name}_cpf_results/{dataset_name}_{model_name}_{interp_tool}_cpf_{seed}_stats.jsonl',
                    seed=seed,
                )
        ...
        if interp_tool == 'linear_probe':
            ...
            if dataset_name in ['TwoHopFact', 'HoppingtooLate', 'SOCRATES']:
                ...
                CPF_result = run_two_hop_linear_probe_evaluation(
                    ...
                    tokenizer=tokenizer,
                    batch_size=batch_size,
                    output_path=f'${SCRATCH}/results/open-r1/twohop_cpf_results/{dataset_name}_{model_name}_{interp_tool}_cpf_{seed}_stats.jsonl',
                    seed=seed,
                )
            else:
                raise ValueError(
...
def accuracy_evaluation(
    ...
    if dataset_name in ['TwoHopFact', 'HoppingtooLate']:
        acc_result = run_two_hop_acc_evaluation(model, dataset, dataset_name, tokenizer, batch_size, output_path=f'${SCRATCH}/results/open-r1/twohop_results/{dataset_name}_{model_name}_{seed}_results.jsonl', seed=seed)

    elif dataset_name in ['Hint_MMLU', 'Hint_GPQA']:
        acc_result = run_hint_acc_evaluation(model, dataset, dataset_name, tokenizer, batch_size, output_path=f'${SCRATCH}/results/open-r1/hint_mmlu_results/hint_mmlu_false_{model_name}_{seed}_results.jsonl', seed=seed)

    elif dataset_name in ['4-digit-Multiplication', '3-digit-Multiplication', '2-digit-Multiplication']:
        acc_result = run_multiplication_acc_evaluation(model, dataset, dataset_name, tokenizer, batch_size, use_cot_prompt=use_cot_prompt, output_path=f'${SCRATCH}/results/open-r1/math_results/{dataset_name}_{model_name}_use-cot-is-{use_cot_prompt}_{seed}_results.jsonl', seed=seed)
