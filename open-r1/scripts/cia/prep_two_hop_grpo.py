...
Usage:
    python scripts/cia/prep_two_hop_grpo.py \
        ...
        --out_dir      ${SCRATCH}/open-r1/datasets/TwoHopFact_cia \
...
def main():
    ...
    ap.add_argument("--dataset_dir", default="${SCRATCH}/datasets")
    ...
    # CoT-inducing prompt (mirrors evaluation_utils.run_two_hop_acc_evaluation
    # at line ~1442). GRPO needs the full prompt baked into `problem` because
    # the trainer feeds it to the model verbatim — otherwise the model gets a
    # bare two-hop question, replies in 1 word, and length_floor pins reward
    # at -1.0 (collapse). Same instructions as eval-time so probe/CoT B_INT
    # signals stay comparable.
    PROMPT_TEMPLATE = (
        "You are an expert in multi-hop factual reasoning. For the following "
        "question, you must reason step by step using exactly two hops. "
        "Always explicitly identify and state the intermediate entity (the "
        "'bridge entity') before giving the final answer. "
        "Structure your response exactly as follows:\n\n"
        "1. First, identify the bridge entity by applying the first implied "
        "relation.\n"
        "2. Then, apply the second relation to the bridge entity to find the "
        "final answer.\n"
        ...
        "Now answer the following question in exactly the same structured "
        "format (steps 1-3, explicitly state the bridge entity):\n"
        "{}"
    )
    ...
    for i, (row, lbl) in enumerate(zip(df.itertuples(index=False), labeled)):
        ...
        records.append({
            "index": i,
            "problem": PROMPT_TEMPLATE.format(str(question)),
            "solution": str(answer),
            "labels": int(b_int),
