...
# breaks DeepSpeed checkpoint resume (DS pickles LossScaler / ZeroOptimizer
# objects). We're loading our own checkpoints from a trusted directory, so
# allowlist the relevant DeepSpeed classes for safe loading.
try:
...
# Belt-and-suspenders: also flip the global default so any other call site
# (e.g. transformers' Trainer pulling LR scheduler state) loads safely.
...
class ClearCacheBeforeSaveCallback(TrainerCallback):
    """Free fragmented GPU memory before checkpoint save to avoid OOM."""
    def on_save(self, args, state, control, **kwargs):
        gc.collect()
        torch.cuda.empty_cache()
        logging.getLogger(__name__).info(
            "ClearCacheBeforeSaveCallback: freed CUDA cache before save"
        )


class SaveAfterTrainingCallback(TrainerCallback):
    """When save_strategy='no', save model AFTER training ends with
    aggressive memory cleanup. Works around OOM during mid-training saves
    when GPU memory is fragmented by rollout KV caches."""

    def on_train_end(self, args, state, control, model=None, **kwargs):
        logger = logging.getLogger(__name__)
        logger.info("SaveAfterTrainingCallback: freeing memory before final save")
        ...
        gc.collect()
        torch.cuda.empty_cache()
        # Use the trainer's own save_model method (handles DeepSpeed).
        trainer = kwargs.get("trainer") or kwargs.get("self")
        if trainer is not None and hasattr(trainer, "save_model"):
            save_path = os.path.join(args.output_dir, "final_model")
            logger.info(f"SaveAfterTrainingCallback: saving to {save_path}")
            trainer.save_model(save_path)
        else:
            # Fallback: save via model.save_pretrained if available.
            save_path = os.path.join(args.output_dir, "final_model")
            if hasattr(model, "save_pretrained"):
                logger.info(f"SaveAfterTrainingCallback: model.save_pretrained → {save_path}")
                model.save_pretrained(save_path)
            else:
                logger.warning("SaveAfterTrainingCallback: cannot find save method")
...
def main(script_args, training_args, model_args):
    ...
    # ------------------------------------------------------------------
    # Qwen3 thinking mode: Qwen3 models default to emitting <think>...
    # </think> blocks that consume the entire completion budget before
    # producing any task output. Disable by injecting enable_thinking=False
    # into every apply_chat_template call.
    # ------------------------------------------------------------------
    if "<think>" in (tokenizer.chat_template or ""):
        ...
        def _apply_no_think(*args, **kwargs):
            ...
            return _orig_apply_chat(*args, **kwargs)
        tokenizer.apply_chat_template = _apply_no_think
        logger.info("Patched tokenizer.apply_chat_template to disable Qwen3 thinking mode")
